...
# GEM (Graph Embedding with Marginalization)

# GRASP + kvantna regresija, deterministički, strukturno
# Varijanta: GEM – ugrađivanje iz marginalizacije (rezidual A − očekivano pod null modelom)
 
"""
Graphs in Space: Graph Embeddings for Machine Learning on Complex Data.
GEM (Graph Embedding with Marginalization): rezidualna matrica
M = A − E[ A ] (Chung–Lu null); SVD od M daje embedding iz „marginalne“ strukture.
"""
...
GEM_ALPHA = 0.5 # 0.5 alpha for GEM
...
def compute_gem_embeddings(M, k=EMBED_DIM):
    A = M[1:40, 1:40].astype(float)
    deg = A.sum(axis=1)
    vol = deg.sum()
    if vol < 1e-12:
        vol = 1.0
    expected = np.outer(deg, deg) / vol
    np.fill_diagonal(expected, 0.0)
    M_res = A - expected
    M_res = M_res + np.abs(M_res.min()) + 1e-9
    U, S_svd, _ = np.linalg.svd(M_res, full_matrices=False)
    n_comp = min(k, len(S_svd))
    emb = U[:, :n_comp] * np.sqrt(np.maximum(S_svd[:n_comp], 1e-12))
    if emb.shape[1] < k:
        emb = np.hstack([emb, np.tile(emb[:, -1:], (1, k - emb.shape[1]))])
    for d in range(k):
        col = emb[:, d]
...
def main():
    ...
    emb = compute_gem_embeddings(M, k=EMBED_DIM)
    ...
    print()
    print("Predikcija (GEM + kvantna regresija, deterministički, strukturno):")
    print(best_combo)
    print()
    print("Score:", pred_scores[best_combo[0]])
    print()
    """
    Predikcija (GEM + kvantna regresija, deterministički, strukturno):
    (2, 8, x, y, z, 33, 34)

    Score: 0.5072225165624945
    """
...
Chung–Lu null: 
E[A_ij] = deg_i · deg_j / vol; dijagonala 0 (39×39).

Rezidual: 
M_res = A − E[A]; pomeramo da bude ne-negativno (plus |min| + 1e-9) (39×39).

Embedding: 
SVD(M_res), prvih k kolona U√Σ (39×k); normalizacija u [0, π] (39×k).
"""
